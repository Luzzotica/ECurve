...
namespace ECC
{
  public static class  ECCUtil 
  {
    ...
    public static byte[] BufferFromBigInt(BigInteger d)
    {
      byte[] buf = d.ToByteArray();

      // If the first byte is not 0, then we are good to go
      if (buf[0] != 0)
      {
        return buf;
      }
      else // Otherwise, we need to strip off the 0
      {
        byte[] buf2 = new byte[buf.Length - 1];
        Array.Copy(buf, 1, buf2, 0, buf2.Length);
        return buf2;
      }
    }
